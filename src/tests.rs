...
fn upload(client: &Client, body: &str) -> String {
...
fn download(client: &Client, id: &str) -> Option<String> {
...
fn delete(client: &Client, id: &str) {
    let id = PasteId::from_param(id).expect("valid ID");
...
fn pasting() {
    ...
    let id_1 = upload(&client, body_1);
    assert_eq!(download(&client, &id_1).unwrap(), body_1);
    ...
    // Upload some unicode.
    let body_2 = "こんにちは";
    let id_2 = upload(&client, body_2);
    assert_eq!(download(&client, &id_2).unwrap(), body_2);
    ...
    let id_3 = upload(&client, body_3);
    assert_eq!(download(&client, &id_3).unwrap(), body_3);
    assert_eq!(download(&client, &id_1).unwrap(), body_1);
    assert_eq!(download(&client, &id_2).unwrap(), body_2);

    // Delete everything we uploaded.
    delete(&client, &id_1);
    assert!(download(&client, &id_1).is_none());

    delete(&client, &id_2);
    assert!(download(&client, &id_2).is_none());

    delete(&client, &id_3);
    assert!(download(&client, &id_3).is_none());
}
